...
function journeyPlanner(startStation, endStation) {
    ...
    if (startLines.length === 0 || endLines.length === 0) {
        return "Please input real station names"
    } else if (startStation === endStation) {
        return "Your trip goes no where?"
    }
...
function consolePrint(origin, destination, journey, numStops) {
    ...
    let string = `origin: ${origin} <br>destination: ${destination} <br><br>${journey}<br><br>${numStops} stops total`
    return string
}
...
let paragraph = document.querySelector("pre")
